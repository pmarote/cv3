	...
	if (!$handle = fopen("{$entry}", 'r')) {
		...
		if (substr($linha, 0, 2) == '01' && substr($linha2, 0, 2) == '02') {
		  ...
		  if (!$handlew = fopen(PR_RESULTADOS . "/{$filewname}", 'w')) {
			werro_die("Nao foi possivel a gravacao do arquivo " . PR_RESULTADOS . "/{$filewname} - Feche o programa ou janela que está o usando<br><br>");
		  }	
		  fputs($handlew, $linha);
		  fputs($handlew, $linha2);
		  while(!feof($handle)) {
			$linha = fgets($handle);
			if (substr($linha, 0, 2) == '02' && substr($linha, 48, 4) > '6000' && substr($linha, 48, 4) < '9999' && substr($linha, 52, 2) == '00')
